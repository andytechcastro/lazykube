...
func (rD *resourceDict) showPortForwardForPod(pod entities.Pod) {
...
func (rD *resourceDict) showPortForwardForDeployment(deploymentName, namespace, contextStr string) {
	...
	rD.App.QueueUpdateDraw(func() {
		...
		modal, list := NewPodSelectionModal(pods, func(pod entities.Pod) {
...
func (rD *resourceDict) showLogsForPod(pod entities.Pod, containerName string) {
	...
	getLogsFn = func(cName string) {
		...
		rD.App.QueueUpdateDraw(func() {
			if err != nil {
				if containerErr, ok := err.(*entities.ContainerSelectionError); ok {
...
func (rD *resourceDict) showExecForPod(pod entities.Pod, containerName string) {
...
func (rD *resourceDict) UpdateResources() {
	...
	switch typeR {
	case "Deployments":
		results, _ = rD.Controller.Deployment.GetFromManyContext(namespaces, contexts)
	case "Pods":
		results, _ = rD.Controller.Pod.GetFromManyContext(namespaces, contexts)
	}
